...
- [v-if](https://vuejs.org/api/built-in-directives.html#v-if): rendert ein Element nur, wenn die Bedingung wahr ist.
    - `<p v-if="flag">请求成功 -- 被v-if控制</p>`; 如果 flag 为 false 这个标签根本不会被创建
...
- [v-show](https://vuejs.org/api/built-in-directives.html#v-show): zeigt/verbirgt ein Element basierend auf einer Bedingung (setzt display: none).
    - `<p v-show="flag">请求成功 --- 被V-show控制</p>` 如果 flag 为 false 这个会被创建, 而且 变为 `<p style="display: none">请求成功 --- 被V-show控制</p>` , 就是 通过 css 的方式被隐藏了 
...
## 2.2 Direktive v-bind (单向绑定)

`<input type="text" v-model="name">`
- v-model 双向绑定: 当页面上的值发生变化的时候 也会影响name 的值. 反过来也一样
- v-bind 单项绑定: 只有当 name 的值发生变化的时候, 才会影像 页面上的 值的显示. 
...
inputValue 为后面 script 中 定义的一个 variable, 他有值
inputValue 必须加 ""  才能被渲染 为他原来本身的值 
...
## 2.3 Direktive v-model  (Two Way Data Binding)
...
## 2.6 Direktive v-on (Event Listener )  and @click
